...
void  BVH::RenderFigure( int drawS, int frame_no, float scale )
{
	RenderFigure( drawS, joints[ 0 ], motion + frame_no * num_channel, scale );
}




void  BVH::RenderFigure( int drawS, const Joint * joint, const double * data, float scale )
{
	glPushMatrix();
	...
	if ( joint->children.size() == 0 ) { 
		RenderBone(joint->index, 0.0f, 0.0f, 0.0f, joint->site[ 0 ] * scale, joint->site[ 1 ] * scale, joint->site[ 2 ] * scale, drawS );
	}
	if ( joint->children.size() == 1 ) {
		Joint *  child = joint->children[ 0 ];
		RenderBone(joint->index, 0.0f, 0.0f, 0.0f, child->offset[ 0 ] * scale, child->offset[ 1 ] * scale, child->offset[ 2 ] * scale, drawS );
	}
	if ( joint->children.size() > 1 ) {
		...
		RenderBone(joint->index, 0.0f, 0.0f, 0.0f, center[ 0 ] * scale, center[ 1 ] * scale, center[ 2 ] * scale, drawS );

		for ( i=0; i<joint->children.size(); i++ ) {
			Joint *  child = joint->children[ i ];
			RenderBone(joint->index, center[ 0 ] * scale, center[ 1 ] * scale, center[ 2 ] * scale,
				child->offset[ 0 ] * scale, child->offset[ 1 ] * scale, child->offset[ 2 ] * scale, drawS );
		}
	}

	for ( i=0; i<joint->children.size(); i++ ) {
		RenderFigure(drawS, joint->children[ i ], data, scale );
	}
...
void  BVH::RenderBone(int flag, float x0, float y0, float z0, float x1, float y1, float z1, int drawS )
{
	...
	if(flag == 1)
		...
		drawHead();
	}else if(flag ==12&& ((drawS==1)||(drawS==2))){
		
		glTranslatef(-0.5, 1.1, 2.5);
		drawSword();
	}else if(flag ==8 && ((drawS==2)||(drawS==3))){

		glTranslatef(0.0, 1.8, 0.0);
		drawShield();

	}else if(flag==2){
		glRotatef(aa++*1, 0.0, 1.0, 0.0);
		glTranslatef(3.0, 1.0, 3.0);
		glRotatef(aa*1, 1.0, 1.0, 1.0);
		drawDiamond();
	}
	else{
		;
